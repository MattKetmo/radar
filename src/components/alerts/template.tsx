...
export function AlertsTemplate(props: Props) {
  ...
  const [alertState, setAlertState] = useQueryState("state", {
    defaultValue: "active",
  });
  ...
  useEffect(() => {
    ...
    const alert = flattenedAlerts.find(
      (a) => a.fingerprint === selectedAlertId
    );
    if (alert) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="overflow-x-clip overflow-y-auto">
        {(alertGroups.length === 0 && loading && <AlertGroupsSkeleton />) || (
